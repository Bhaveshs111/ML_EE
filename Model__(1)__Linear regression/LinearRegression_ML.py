...
def main(user_name):
    ...
    Exp=np.array(x_list)              #[[1] [2] [3] ...] in 2D array
    salary=np.array(y_list)
    ...
    myobj_regression.fit(x_train,y_train)     #args are arrays in sklearn..
    ...
    while(True):
        ...
        print(f"TotalPowewr(gwh): {myobj_regression.predict(np.array([[x_newdata]]))}")
