...
def Table(x_test,y_test,y_predict):
        ...
        for i in range(len(x_test)):
            table.add_row([x_test_list[i],round(float(y_predict_list[i]),2),y_test_list[i]])
        print(table)
...
def main(user_name):
    ...
    x=np.array(x)                #[[1] [2] [3] ...] in 2D array
    y=np.array(y)
    ...
    print("would you like to see the polynomial data ?")
    ...
    if ans==1:
        ...
        if pf<0.10:
            ...
            while(True):
                ...
                print(f"TotalPowewr(gwh): {round(float(myobj_regression.predict(x_n)),2)}")
