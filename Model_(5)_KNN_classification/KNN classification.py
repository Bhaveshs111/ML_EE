...
def Table(x_test,y_test,y_predict):
        table=PT()
        table.field_names=["lsf,voltage:","predicted value of cluster:","Actual value of cluster:"]
...
def main(user_name):
    ...
    print("Now you can predict the cluster using this model")
        
    while(True):
        try:
            print("\nEnter the data with [x,y] format:")
            x_newdata=eval(input())
            print(f"cluster number: {classifier_obj.predict(np.array([x_newdata]))}")
